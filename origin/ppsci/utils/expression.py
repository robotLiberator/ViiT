...
class ExpressionSolver(nn.Layer):
    ...
    def eval_forward(
        ...
        # print(float(validator_loss))
      
        if (float(validator_loss)==float("inf")):
            print(float(validator_loss))
            # 将Paddle张量转换为NumPy数组
            tensor_dict = output_dict

            # 指定要保存的txt文件路径
            file_path = './output/hrrr_finetune/test/inf_output_data.h5'

            # 从字典中提取张量，并将它们存储在列表中
            tensor_list = [tensor for key, tensor in tensor_dict.items()]

            # 合并张量列表为一个48维的张量
            combined_tensor = paddle.concat(tensor_list, axis=0)
